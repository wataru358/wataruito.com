...
// @todo:
// this does not work in a way I want at this time.
// maybe we need to use canvas with pixi or something...
// expose doms 100%, use react just to mount...
...
class CellAutomataRev extends Component {
  constructor () {
    super();
  ...
  update(){
    ...
    while (cell != null) {
      ...
      cell.bufferR += cell.bufferRVel;
			...
			if (cell.bufferR < 0) {
				cell.bufferR = cell.bufferR + 100;
				cell.bufferRVel *= -1;
			} else if (cell.bufferR > 100) {
				cell.bufferR = cell.bufferR - 100;
				cell.bufferRVel *= -1;
			}
      cell = cell.next;
    ...
    cell = cellList.first;
		while (cell != null) {
			//cell.r = cell.bufferR + cell.r;
      cell.r = cell.bufferR;
			cell.rVel = cell.bufferRVel;
      /*if(cell.animationDurationStack.length < 10) {
        cell.animationDurationStack.push(Math.floor(6 / cell.r)+'s');
      }*/
  ...
  cell(cell, cellIndex, rowIndex) {
    return (
      <div
        className={styles.cell + ' ' + 'cellAnime'}
        onAnimationEnd={(e) => {
            //console.log(e);
            e.persist();
            e.currentTarget.style.animationDuration = (e.currentTarget.dataset.nextDuration / 10) + 's';

            e.currentTarget.className = e.currentTarget.className.replace(/cellAnime/i,'');

            var target = e.currentTarget;
            setTimeout(function(){
              target.className = target.className + ' cellAnime'

            },1);

            //e.currentTarget.className = e.currentTarget.className + ' cellAnime'
            /*setTimeout(function(){

              e.currentTarget.className = e.currentTarget.className + ' cellAnime'

            }, 1);*/
            //e.currentTarget.classList.remove('cellAnime');
            //setTimeout(function(){e.currentTarget.classList.add('cellAnime')} ,1);

          }
        }
        key={rowIndex.toString() + '_' + cellIndex.toString()}
        data-next-duration={cell.r}
        style={
          {
            flexBasis:this.state.baseSize,
            height:this.state.baseSize,
            flexShrink:0,
            animationDuration: '1s'
            /*animationDuration: Math.floor(6 / cell.r)+'s'*/
            /*transform:'rotate( '+Math.floor(cell.r)+'deg )',*/
            /*WebkitTransitionDuration:'3s',
            OTransitionDuration:'3s',
...
export default CellAutomataRev
